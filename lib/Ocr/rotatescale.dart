...
class ScaleRotateUpdateDetails {
  ...
  @override
  String toString() =>
      'ScaleRotateUpdateDetails(focalPoint: $focalPoint, scale: $scale, rotation: $rotation)';
}
...
class ScaleRotateGestureRecognizer extends OneSequenceGestureRecognizer {
  ...
  ScaleRotateGestureRecognizer({Object debugOwner})
      ...
      if (onEnd != null) {
        ...
        if (_isFlingGesture(velocity)) {
          ...
          invokeCallback<void>('onEnd',
              () => onEnd(new ScaleRotateEndDetails(velocity: velocity)));
        } else {
          invokeCallback<void>('onEnd',
              () => onEnd(new ScaleRotateEndDetails(velocity: Velocity.zero)));
        }
      }
  ...
  void _advanceStateMachine(bool shouldStartIfAccepted) {
    if (_state == _ScaleRotateState.ready) _state = _ScaleRotateState.possible;
    ...
    if (_state == _ScaleRotateState.started && onUpdate != null)
      invokeCallback<void>(
          'onUpdate',
          () => onUpdate(new ScaleRotateUpdateDetails(
              scale: _scaleFactor,
  ...
  void _dispatchOnStartCallbackIfNeeded() {
    ...
    if (onStart != null)
      invokeCallback<void>(
          'onStart',
          () => onStart(
              new ScaleRotateStartDetails(focalPoint: _currentFocalPoint)));
  }
